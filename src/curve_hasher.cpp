...
//debug
CurveHasher::CurveHasher(int numDimension,int min,int max) {
    this->numDimension = numDimension;
    this->max = max;
    gridSize = 4 * numDimension * min;
    //TODO:set variables properly
    phasher = new PointHasher(3,max,20);
    random_device r;
    //do we need another engine?
    default_random_engine e1(r());
    uniform_real_distribution<double> uniform_dist(0,numDimension);
...
double *CurveHasher::snap(Point *point) {
    ...
    for (int i = 0; i< numDimension; i++) {
        snapedPoint[i] = roundCoordinate(point,i);
    }
    return snapedPoint;
}
...
double CurveHasher::roundCoordinate(Point *point,int i) {
    double _gridSize = double(gridSize);
    double round =  point->getCoordinate(i) + _gridSize/2.0;
    round -= fmod(round,_gridSize);
    double shiftDown = round - (_gridSize - shiftedGrid[i]);
    double shiftUp = round + shiftedGrid[i];
    if (fabs(point->getCoordinate(i) - shiftUp) < fabs(point->getCoordinate(i)- shiftDown)) {
        return shiftUp;
    }
    return shiftDown;
}
